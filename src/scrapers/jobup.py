...
JOBUP_BASE_URL = "https://www.jobup.ch"
SOURCE = "jobup"
...
class JobupScraper(BaseScraper):
    """Scraper Playwright pour jobup.ch."""
    ...
    async def _fetch_search_page(self, browser: Browser, url: str) -> list[OfferStub]:
        ...
        return await self._rate_limited(
            url,
            lambda: self._with_retry(f"jobup_search:{url}", operation),
        )
    ...
    async def _parse_listing_with_browser(self, browser: Browser, stub: OfferStub) -> Offer:
        ...
        return await self._rate_limited(
            url,
            lambda: self._with_retry(f"jobup_detail:{url}", operation),
        )
...
def build_search_url(term: str, location: str) -> str:
    """Construit une URL jobup stable avec paramètres lisibles."""

    return (
        f"{JOBUP_BASE_URL}/fr/emplois/"
        f"?term={quote_plus(term)}&location={quote_plus(location)}&page=1"
    )
...
async def dismiss_cookie_banner(page: Page) -> None:
    ...
    labels = ("Tout accepter", "Accepter", "Accept all", "Alle akzeptieren")
    for label in labels:
        ...
        try:
            ...
            get_logger("scrapers.jobup").warning(
...
async def extract_search_cards(page: Page) -> list[OfferStub]:
    ...
    for index in range(count):
        ...
        if href is None:
            continue
        absolute_url = urljoin(JOBUP_BASE_URL, href)
...
class JobupHTMLParser(HTMLParser):
    """Petit extracteur HTML suffisant pour fixtures et fallback sans dépendance."""
...
def parse_search_html(html: str) -> list[OfferStub]:
    """Parse une page de recherche jobup depuis du HTML déjà capturé."""

    parser = JobupHTMLParser()
    parser.feed(html)
    stubs: list[OfferStub] = []
    seen_urls: set[str] = set()
...
def parse_offer_html(html: str, fallback_stub: OfferStub | None = None) -> Offer:
    """Parse une page détail jobup depuis du HTML, avec priorité au JSON-LD."""
    ...
    url = str(fallback_stub.url) if fallback_stub else parser.meta.get("og:url", JOBUP_BASE_URL)
    title = parser.meta.get("og:title") or (fallback_stub.title if fallback_stub else "Offre jobup")
    ...
    for block in blocks:
        ...
        for node in nodes:
            if not isinstance(node, dict):
                continue
            typed_node = node
            if typed_node.get("@type") != "JobPosting":
                continue
            return offer_from_jobposting(typed_node, fallback_stub)
    return None


def offer_from_jobposting(payload: dict[str, Any], fallback_stub: OfferStub | None) -> Offer:
    url = str(payload.get("url") or (fallback_stub.url if fallback_stub else JOBUP_BASE_URL))
    title = str(payload.get("title") or (fallback_stub.title if fallback_stub else "Offre jobup"))
...
def offer_from_parts(
    *,
    ...
    normalized_url = urljoin(JOBUP_BASE_URL, url)
...
def is_job_detail_url(url: str) -> bool:
    return "/emplois/detail/" in url or "/jobs/detail/" in url
...
async def async_main(max_results: int) -> None:
    configure_logging()
    logger = get_logger("scrapers.jobup")
    ...
    logger.info("jobup_scrape_done", count=len(offers))


def main() -> None:
    parser = argparse.ArgumentParser(description="Scrape jobup.ch")
