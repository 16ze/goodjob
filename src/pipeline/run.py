...
from src.scrapers.company_sites import ALL_COMPANY_CONFIGS, CompanySiteScraper
...
async def async_main(max_results: int, dry_run: bool) -> None:
    ...
    company_scrapers = [CompanySiteScraper(cfg) for cfg in ALL_COMPANY_CONFIGS]
    scrapers: list[OfferScraper] = [JobupScraper(), JobsScraper(), IndeedScraper(), *company_scrapers]
    for scraper in scrapers:
        await ingest_jobup_offers(scraper, repository, query=query, dry_run=dry_run)


def main() -> None:
    parser = argparse.ArgumentParser(description="Ingestion toutes sources vers Supabase.")
